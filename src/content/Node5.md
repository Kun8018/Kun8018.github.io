...
react中fiber架构，同样如果支持MessageChannel优先选择MessageChannel，不支持采用setTimeout降级处理。因为MessageChannel 是一对一通信，没有像setTimeout受到4ms限制，还有rAF受到浏览器渲染频率限制(16.6ms)
...
微任务和宏任务是绑定的，每个宏任务在执行时，会创建自己的微任务队列。在一个宏任务执行结束后，所有的微任务都会执行完毕(渲染前)

#### 事件循环与UI渲染

由于浏览器中Js线程与ui线程互斥，所以js执行会堵塞UI渲染

js与UI渲染线程的执行机制是，

`同步任务` > `微任务` > `requestAnimationFrame` > `DOM渲染` > `宏任务`

所以微任务和同步任务会堵塞UI渲染，也有人说requestAnimationFrame的执行时机的是慢于同步任务，但是和宏任务的执行时机是不确定的

验证demo：https://codepen.io/jackluson/pen/eYVgewE
...
let tasts = [axios(),axios(),axios(),axios()];
