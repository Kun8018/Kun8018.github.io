...
在Python函数中`import`语句用于导入其他模块的功能，实现代码复用，可以放在函数内部（局部导入）或文件顶部（全局导入），局部导入可以避免命名冲突、按需加载，全局导入则使模块全局可用；常用方式包括 `import module`、`from module import item`、`import module as alias`、`from module import *`，全局导入推荐按`PEP 8`规范放在文件开头。 

1. 导入方式与区别

- **`import module`**: 导入整个模块，使用时需 `module.function()`。
- **`from module import function`**: 只导入特定函数或类，可直接使用 `function()`，避免重复输入模块名，但可能引起命名冲突。
- **`import module as alias`**: 导入模块并设置别名，使用 `alias.function()`，简化长模块名，如 `import numpy as np`。
- **`from module import \*`**: 导入模块所有内容，直接使用，但不推荐，易污染命名空间。 
- 函数内导入 (局部导入)

- **作用**：仅在函数内部导入，对该函数可见。
- **优点**：
  - **按需加载**：只有调用函数时才加载模块，节省资源。
  - **避免冲突**：解决不同模块中同名函数或变量的冲突。

文件全局导入

- **作用**：模块内容在整个文件中可用，是 Python 标准做法。
- **优点**：代码结构清晰，易于追踪依赖。

将所有 `import` 语句放在文件顶部。

- 按标准库、第三方库、本地库的顺序分组导入。
- 每个 `import` 语句独占一行（除非导入多个同类模块
